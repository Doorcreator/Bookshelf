...
function bkup_fld {
    ...
    for f in $(ls $des_folder);do
        local fmsize=$(echo $(du -h "$des_folder"/"$f") | awk '{print $1}')
        git add "$des_folder"/"$f"
        git commit -m "${fmsize}bytes"
        git push bksf master
        if [ $? -eq 0 ];then
            COUNT_PUSH=$(( $COUNT_PUSH+1 ))
            rm -f "$des_folder"/"$f";fi
    done
    rmdir "$des_folder"
    echo $COUNT_LNK links made, $COUNT_CMPR files compressed, and $COUNT_PUSH files pushed successfully.
}

# bkup_fld /g/cs/batpush /g/cs/backup2git/batpush
bkup_fld /g/testfd /g/cs/backup2git/testfd
